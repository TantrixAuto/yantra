...
This is the most significant feature of Yantra: achieving deferred, top-down semantic actions on top of LALR(1) parsing. Top-down (LL-style) parsers get this naturally, since they already build the tree top-down as they parse. LALR(1) parsers are bottom-up, and normally don't: actions run mid-parse, as each rule reduces, with no way to see the shape of a node's children before deciding what to do with them. Yantra keeps LALR(1)'s bottom-up construction, and all the time and space efficiency that comes with it, while still giving you a top-down walking experience, by building the full AST first and walking it separately afterward.

The generated parser (a.out) uses this process to parse input files and execute the semantic actions.

In Yacc, Bison, and Lemon, the parser reads the input and invokes the semantic actions associated with each rule as and when the rule is reduced.

The semantic actions will typically be used to generate an AST. Then the AST is traversed and the required output is generated. The developer has to write, by hand, the code to define the AST data structures and the code to walk the AST.
...
#### Parser generated by Yacc/Bison/Lemon-style parser generators
```mermaid
flowchart LR

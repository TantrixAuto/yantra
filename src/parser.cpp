...
struct Parser {
    ...
    struct RuleSetType {
        ...
        const yg::Grammar::Rule* rule = nullptr;

        /// @brief The associated Walker
        yg::Grammar::Walker* walker = nullptr;
    ...
    inline void addRule(const Token& t, const Token& name, std::unique_ptr<yg::Grammar::Rule>& rule, const bool& anchorSet) {
        assert(rule);
    ...
    inline std::unique_ptr<yg::Grammar::Atom> make_atom(yg::Grammar::Atom_t&& a) {
        return std::make_unique<yg::Grammar::Atom>(std::move(a));
    }

    /// @brief create a regex atom instance and advance the lexer to next token
    inline std::unique_ptr<yg::Grammar::Atom> make_atom_leaf(yg::Grammar::Atom_t&& a) {
        lexer.next();
    ...
    inline yg::Grammar::Primitive::Atom_t primitive_atomx(const Token& t) {
    ...
    inline std::unique_ptr<yg::Grammar::Atom> primitive_atom() {
        ...
        return make_atom_leaf(yg::Grammar::Primitive(t.pos, std::move(ax)));
    }

    /// @brief read a regex class
    /// [a-z], [^a-z], etc
    inline std::unique_ptr<yg::Grammar::Atom> class_atoms() {
        ...
        if((t = peek(tr)).id == Token::ID::RX_CLASS_ENTER) {
            ...
            std::vector<yg::Grammar::Primitive::Atom_t> atoms;
            while((t = peek(tr)).id != Token::ID::RX_CLASS_LEAVE) {
                ++classDepth;
                yg::Grammar::Primitive::Atom_t rhs = primitive_atomx(t);
                --classDepth;
                atoms.push_back(rhs);
                lexer.next();
            }
            return make_atom_leaf(yg::Grammar::Class{t.pos, negate, std::move(atoms)});
        }
    ...
    /// @brief read a regex class
    /// (abc), (!abc), etc
    inline std::unique_ptr<yg::Grammar::Atom> group_atoms() {
        ...
        if((t = peek(tr)).id == Token::ID::RX_GROUP_ENTER) {
            ...
            return make_atom_leaf(yg::Grammar::Group{t.pos, capture, std::move(atom)});
        }
        return class_atoms();
    }
    ...
    /// a*, a+, a?, etc
    inline std::unique_ptr<yg::Grammar::Atom> atom_closure() {
        ...
        if((t = peek(tr)).id != Token::ID::RX_DBLQUOTE) {
            ...
            case Token::ID::RX_CLOSURE_STAR: {
                lhs = make_atom_leaf(yg::Grammar::Closure{grammar, t.pos, std::move(lhs), 0, grammar.maxRepCount});
                break;
            }
            case Token::ID::RX_CLOSURE_PLUS: {
                lhs = make_atom_leaf(yg::Grammar::Closure{grammar, t.pos, std::move(lhs), 1, grammar.maxRepCount});
                break;
            }
            case Token::ID::RX_CLOSURE_QUESTION: {
                lhs = make_atom_leaf(yg::Grammar::Closure{grammar, t.pos, std::move(lhs), 0, 1});
                break;
            }
            case Token::ID::RX_CLOSURE_ENTER: {
                ...
                lhs = make_atom_leaf(yg::Grammar::Closure{grammar, t.pos, std::move(lhs), min, max});
                break;
            }
            default:
                return lhs;
    ...
    /// ab, (abc)(def), etc
    inline std::unique_ptr<yg::Grammar::Atom> atom_and_atom() {
        ...
        while((t = peek(tr)).id != Token::ID::RX_DBLQUOTE) {
            ...
            case Token::ID::RX_ESC_CLASS_HEX: {
                auto rhs = atomx();
                lhs = make_atom(yg::Grammar::Sequence{t.pos, std::move(lhs), std::move(rhs)});
                break;
            }
            default:
                return lhs;
    ...
    /// a|b, (abc)|(def), etc
    inline std::unique_ptr<yg::Grammar::Atom> atom_or_atom() {
        ...
        while((t = peek(tr)).id == Token::ID::RX_DISJUNCT) {
            lexer.next();
            auto rhs = atomx();
            lhs = make_atom(yg::Grammar::Disjunct{t.pos, std::move(lhs), std::move(rhs)});
        }
        return lhs;
    }

    /// @brief read a regex atom recursively, incrementing the current lvl
    inline std::unique_ptr<yg::Grammar::Atom> atomx() {
        Tracer tr{lvl, "atomx"};
        ++lvl;
    ...
    inline void walkers() {
        ...
        while((t = peek(tr)).id == Token::ID::ID) {
            ...
            // ^
            auto walkerID = t;
            const yg::Grammar::Walker* base = nullptr;
    ...
    inline void walker_traversal() {
        ...
        if(t.text == "manual") {
            walker->setTraversalMode(yg::Grammar::Walker::TraversalMode::Manual);
        }else if(t.text == "top_down") {
            walker->setTraversalMode(yg::Grammar::Walker::TraversalMode::TopDown);
        }else{
    ...
    inline void set_precedence(const yg::Grammar::RegexSet::Assoc& assoc, const Token& s) {
        Tracer tr{lvl, s.text};
    ...
    inline void begin_pragma() {
        ...
        if(t.text == "left") {
            return set_precedence(yg::Grammar::RegexSet::Assoc::Left, t);
        }

        if(t.text == "right") {
            return set_precedence(yg::Grammar::RegexSet::Assoc::Right, t);
        }

        if(t.text == "token") {
            return set_precedence(yg::Grammar::RegexSet::Assoc::None, t);
        }
    ...
    inline void parse_node(yg::Grammar::Rule& rule) {
        ...
        auto& node = rule.addNode(t.pos, t.text, yg::Grammar::Node::NodeType::RegexRef);
        if(isRegexName(t.text) == true) {
            node.type = yg::Grammar::Node::NodeType::RegexRef;
        }else if(isRuleName(t.text) == true) {
            node.type = yg::Grammar::Node::NodeType::RuleRef;
        }else{
    ...
    inline void parse_rule(const Token& ruleName) {
        ...
        lexer.next();
        auto rule = std::make_unique<yg::Grammar::Rule>();
    ...
    inline void parse_regex(const Token& ruleName) {
        ...
        Token t = peek(tr);
        auto assoc = yg::Grammar::RegexSet::Assoc::Right;
        switch(t.id) {
        case Token::ID::COLONEQ:
            assoc = yg::Grammar::RegexSet::Assoc::Right;
            break;
        case Token::ID::COLONEQGT:
            assoc = yg::Grammar::RegexSet::Assoc::Left;
            break;
        case Token::ID::COLONEQEQ:
            assoc = yg::Grammar::RegexSet::Assoc::None;
            break;
        default:
            throw GeneratorError(__LINE__, __FILE__, t.pos, "INVALID_INPUT");
        }

        auto regex = std::make_unique<yg::Grammar::Regex>();
        ...
        if(t.id == Token::ID::LSQUARE) {
            ...
            regex->modeChange = yg::Grammar::Regex::ModeChange::Init;
            regex->nextMode = "";

            if(t.id == Token::ID::ID) {
                regex->modeChange = yg::Grammar::Regex::ModeChange::Next;
                regex->nextMode = t.text;
                lexer.next();
                t = peek(tr);
            }else if(t.id == Token::ID::CARET){
                regex->modeChange = yg::Grammar::Regex::ModeChange::Back;
                regex->nextMode = "";
                lexer.next();
                t = peek(tr);
            }
...
void parseInput(yg::Grammar& g, Stream& is) {
    ...
    g.addRegexByName(g.end, yg::Grammar::RegexSet::Assoc::Right);
    g.addRegexByName(g.empty, yg::Grammar::RegexSet::Assoc::Right);
    ...
    std::vector<yg::Grammar::Regex*> unuseds;
